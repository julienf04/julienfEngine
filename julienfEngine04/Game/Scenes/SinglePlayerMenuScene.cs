...
namespace julienfEngine1
{
    class SinglePlayerMenuScene : Scene
    {
        ...
        public override void Awake()
        {
            ...
            int normalSinglePlayerMenuPosY = easySinglePlayerMenuPosY + _DISTANCE_BETWEEN_BUTTONS_POSY;
            int hardSinglePlayerMenuPosY = normalSinglePlayerMenuPosY + _DISTANCE_BETWEEN_BUTTONS_POSY;
            int backPosY = hardSinglePlayerMenuPosY + _DISTANCE_BETWEEN_BUTTONS_POSY;

            EasySinglePlayerMenu easySinglePlayerMenu = new EasySinglePlayerMenu(buttonsPosX, easySinglePlayerMenuPosY, true, true, 0);
            NormalSinglePlayerMenu normalSinglePlayerMenu = new NormalSinglePlayerMenu(buttonsPosX, normalSinglePlayerMenuPosY, true, true, 0);
            HardSinglePlayerMenu hardSinglePlayerMenu = new HardSinglePlayerMenu(buttonsPosX, hardSinglePlayerMenuPosY, true, true, 0);
            Back back = new Back(buttonsPosX - 2, backPosY, true, true, 0); // -2 becouse this font is not the same, so it looks more aligned

            _buttonsMainMenu = new IClickable[4]
            {
                easySinglePlayerMenu,
                normalSinglePlayerMenu,
                hardSinglePlayerMenu,
                back
            };


            int arrowMenuPosX = buttonsPosX + easySinglePlayerMenu.P_GameObjectFigures[0].P_Figure[0].Length + _DISTANCE_BETWEEN_BUTTONS_AND_ARROW_POSX;

            _arrowMenu = new ArrowMenu(_buttonsMainMenu, arrowMenuPosX, easySinglePlayerMenuPosY, true, true, 0, ArrowMenu.RO_FigureMenuArrow, (byte)ArrowMenu.E_ArrowSidesAndSizes.BigArrowPointLeft);
